function gmapsInitialize(gmapElement) {
   // Make sure that google maps has been loaded before attempting to proceed
...
if(gmaps.length > 0) {
   ...
   var client = gmaps.data('configClient');
   var channel = gmaps.data('configChannel');
   ...
   if(configKey) {
      ...
      scriptPromise = $.ajax({
         async: false,
         url: "https://maps.googleapis.com/maps/api/js?v=3.exp&signed_in=true&client=" + client + "&channel=" + channel,
         dataType: "script",
         cache: true,
         error: function () {
            console.log("ajax failure");
